...
/-
The match expression is simplified by binding "global" names 
to the type arguments by declaring them before the colon. Either
way is fine.
-/
def glue_funs'' 
  {α β γ : Type} :  
...
/-
Lean also has the (not universally loved) "feature" of
assuming types for lower case letters used as argument 
names (including Greek letters). I suggest that you not
use this feature.
-/
...
#eval glue_funs double double 5 -- expect 20
#eval glue_funs square square 5 -- expect 625

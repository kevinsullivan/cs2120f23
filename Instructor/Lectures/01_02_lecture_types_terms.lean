/-!
# Lectures 1 and 2: Types and Terms, Applications
-/
...
#check Bool → (Bool → Bool) -- → is right associative
#check (and)                -- same type
...
/-!
Here we used #eval instead of #reduce to force evaluation.
They do the same thing but use different evaluators. You will
use one or the other depending on context.
-/
...
we just saw, *String → String → String*, is exactly the same
as this type!
-/
#check String → (String → String)
...
/-!
Here's the type of any function that takes two arguments, a
*function* (from String to String), the second being a String,
with the function finally returning a String result.
-/
...
then apply the first argument (the -appending! function)
to the second argument, "Hello," and finally return the
string "Hello!".
-/

/-!
## Function Terms
Let's check the type types of some built-in terms with
function types.
-/
...
#check (Nat.pred)       -- Subtracts 1 from its argument; zero goes to zero

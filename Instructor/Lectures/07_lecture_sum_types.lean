...
and building a term around it, and the other taking a
β value and doing the same.

We'll use the constructor names *inl* and *inr*, short
for *introduction on the left* and *introduction on the
right,* where *inl* takes an argument of type α and *inr*
takes an argument of type β. So, for example, if *(a : α)*,
then *Sum.inl a* will be an object of a sum type; and if
*(b : β)* then *Sum.inr b* will be a value of the same
type.

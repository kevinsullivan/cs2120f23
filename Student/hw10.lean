...
variable
  (Dog : Type)
  (Cat : Type)
  (dogLikesCat : Dog → Cat → Prop)
  (dogLikesDog : Dog → Dog → Prop)
  (catLikesCat : Cat → Cat → Prop)
  (c : Cat)
  (d : Cat)
  (pf_c : ∀ (c d : Cat), catLikesCat c d)

-- 1. Every dog likes some cat.

#check ∀ (d : Dog), ∃ (c : Cat), dogLikesCat d c

-- 2. If any dog, d, likes any dog, g, and that dog, g, likes any dog, w, then d likes w.

#check ∀ (d w g : Dog), dogLikesDog d g → dogLikesDog g w → dogLikesDog d w

-- 3. If any cat, c, likes any cat, d, then d also likes c.

#check ∀ (c d : Cat), catLikesCat c d → catLikesCat d c

-- 4. Every cat, c, likes itself.

#check ∀ (c : Cat), catLikesCat c c
-- or
#check ∃ (c : Cat), catLikesCat c c

-- 5. If every cat likes every other cat, and c and d are cats, then c likes d.

#check ∀ (c d : Cat), catLikesCat c d
